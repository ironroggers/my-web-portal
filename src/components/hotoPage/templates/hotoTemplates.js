// HOTO Field Templates
export const HOTO_TEMPLATES = {
  block: {
    name: "Block HOTO Template",
    description: "Standard template for Block level HOTO operations",
    hotoType: "block",
    fields: [
      {
        sequence: 1,
        key: "Asset Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 2,
        key: "Item Description",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 3,
        key: "Asset make",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 4,
        key: "Asset Model",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 5,
        key: "Serial Number",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 6,
        key: "MAC Address",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 7,
        key: "IP Address",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 8,
        key: "Subnet",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 9,
        key: "Firm ware",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 10,
        key: "No of PON Ports",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 11,
        key: "Uplink Ports",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 12,
        key: "Power Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 13,
        key: "No Of WAN Ports",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 14,
        key: "No Of LANPorts",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 15,
        key: "Switch Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 16,
        key: "No Of Ports",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 17,
        key: "VLAN Support",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "dropdown"
      },
      {
        sequence: 18,
        key: "PoE Support",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "dropdown"
      },
      {
        sequence: 19,
        key: "ONT Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 20,
        key: "No of Ethernet Ports",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 21,
        key: "WiFi Capability",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "dropdown"
      },
      {
        sequence: 22,
        key: "Rack Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 23,
        key: "SizeU",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 24,
        key: "Manufacture",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 25,
        key: "Power Source",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 26,
        key: "Battery Backup",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 27,
        key: "Cooling System",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 28,
        key: "Capacity kVA",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 29,
        key: "Input Voltage",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 30,
        key: "Output Voltage",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 31,
        key: "Capacity Ah",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 32,
        key: "Voltage",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 33,
        key: "Battery Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 34,
        key: "Fuel Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 35,
        key: "Power Rating W",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 36,
        key: "Surge Protection",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "dropdown"
      },
      {
        sequence: 37,
        key: "Panel Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 38,
        key: "Battery Integration",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "dropdown"
      },
      {
        sequence: 39,
        key: "Capacity FiberPortsSplices",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 40,
        key: "No Of Input Output Ports",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 41,
        key: "FDMS Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 42,
        key: "Rack Mount",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "dropdown"
      },
      {
        sequence: 43,
        key: "Enclosure Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 44,
        key: "Ingress Protection",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 45,
        key: "No Of Adapter Panels",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 46,
        key: "No Of Splice Trays",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 47,
        key: "Cable Entry Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 48,
        key: "Patch Cord Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 49,
        key: "No Of Patch Cords Installed",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 50,
        key: "Fiber Core Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 51,
        key: "Connector Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 52,
        key: "Fiber Termination Method",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 53,
        key: "Remarks",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      }
    ]
  },
  gp: {
    name: "GP HOTO Template",
    description: "Standard template for GP level HOTO operations",
    hotoType: "gp",
    fields: [
      {
        sequence: 1,
        key: "Optical Power in dB",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "media"
      },
      {
        sequence: 2,
        key: "Ring Code",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 3,
        key: "Route Code",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 4,
        key: "LGD Code",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 5,
        key: "Contact Person",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 6,
        key: "Email",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "email"
      },
      {
        sequence: 7,
        key: "Mobile",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "phone"
      },
      {
        sequence: 8,
        key: "ONTs Installed",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "dropdown"
      },
      {
        sequence: 9,
        key: "litup Yes No",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "dropdown"
      },
      {
        sequence: 10,
        key: "Phase 1 or 2",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "dropdown"
      },
      {
        sequence: 11,
        key: "OTDR status",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "dropdown"
      },
      {
        sequence: 12,
        key: "OTDR status date",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "date"
      },
      {
        sequence: 13,
        key: "Ring topology",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "dropdown"
      },
      {
        sequence: 14,
        key: "LPSM status",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "media"
      },
      {
        sequence: 15,
        key: "Fiber core 12 24 48 96",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "dropdown"
      },
      {
        sequence: 16,
        key: "op received GP date",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "media"
      },
      {
        sequence: 17,
        key: "op transmitted from block dB",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "media"
      },
      {
        sequence: 18,
        key: "op transmitted from block date",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "media"
      },
      {
        sequence: 19,
        key: "splice loss dB",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "media"
      },
      {
        sequence: 20,
        key: "remarks",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 21,
        key: "Asset make",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 22,
        key: "Asset Model",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 23,
        key: "Serial Number",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 24,
        key: "MAC Address",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 25,
        key: "IP Address",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 26,
        key: "Subnet",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 27,
        key: "Firm ware",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 28,
        key: "No of PON Ports",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 29,
        key: "Uplink Ports",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 30,
        key: "Power Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 31,
        key: "No Of WAN Ports",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 32,
        key: "No Of LANPorts",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 33,
        key: "Switch Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 34,
        key: "No Of Ports",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 35,
        key: "VLAN Support",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "dropdown"
      },
      {
        sequence: 36,
        key: "PoE Support",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "dropdown"
      },
      {
        sequence: 37,
        key: "ONT Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 38,
        key: "No of Ethernet Ports",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 39,
        key: "WiFi Capability",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "dropdown"
      },
      {
        sequence: 40,
        key: "Rack Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 41,
        key: "SizeU",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 42,
        key: "Manufacture",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 43,
        key: "Power Source",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 44,
        key: "Battery Backup",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 45,
        key: "Cooling System",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 46,
        key: "Capacity kVA",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 47,
        key: "Input Voltage",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 48,
        key: "Output Voltage",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 49,
        key: "Capacity Ah",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 50,
        key: "Voltage",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 51,
        key: "Battery Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 52,
        key: "Fuel Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 53,
        key: "Power Rating W",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 54,
        key: "Surge Protection",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "dropdown"
      },
      {
        sequence: 55,
        key: "Panel Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 56,
        key: "Battery Integration",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "dropdown"
      },
      {
        sequence: 57,
        key: "Capacity FiberPortsSplices",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 58,
        key: "No Of Input Output Ports",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 59,
        key: "FDMS Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 60,
        key: "Rack Mount",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "dropdown"
      },
      {
        sequence: 61,
        key: "Enclosure Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 62,
        key: "Ingress Protection",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 63,
        key: "No Of Adapter Panels",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 64,
        key: "No Of Splice Trays",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 65,
        key: "Cable Entry Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 66,
        key: "Patch Cord Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 67,
        key: "No Of Patch Cords Installed",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 68,
        key: "Fiber Core Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 69,
        key: "Connector Type",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 70,
        key: "Fiber Termination Method",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 71,
        key: "Remarks",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      }
    ]
  },
  ofc: {
    name: "OFC HOTO Template", 
    description: "Standard template for OFC level HOTO operations",
    hotoType: "ofc",
    fields: [
      {
        sequence: 1,
        key: "FPOI",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "text"
      },
      {
        sequence: 2,
        key: "FPOI Closure Available",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "dropdown"
      },
      {
        sequence: 3,
        key: "FPOI Photo",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "media"
      },
      {
        sequence: 4,
        key: "OTDR Length FPOI to GP Mtr",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 5,
        key: "OTDR Trace FPOI to GP",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "media"
      },
      {
        sequence: 6,
        key: "No of Overhead Section Segment",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 7,
        key: "Overhead Section Stretch Mtr",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 8,
        key: "Overhead Details From",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "media"
      },
      {
        sequence: 9,
        key: "Overhead Details To",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "media"
      },
      {
        sequence: 10,
        key: "L14 Diagram",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "media"
      },
      {
        sequence: 11,
        key: "ROW Details Documents",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "media"
      },
      {
        sequence: 12,
        key: "No of Joints Section",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 13,
        key: "Chamber Count",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "number"
      },
      {
        sequence: 14,
        key: "Chamber Coordinates Photos",
        value: "",
        confirmation: false,
        remarks: "",
        status: 1,
        fieldType: "media"
      }
    ]
  },
  custom: {
    name: "Custom Template",
    description: "Start with a blank template and add custom fields",
    hotoType: "",
    fields: []
  }
};

// Template field type options
export const FIELD_TYPE_OPTIONS = [
  { value: "text", label: "Text" },
  { value: "number", label: "Number" },
  { value: "dropdown", label: "Dropdown" },
  { value: "media", label: "Media Upload" },
  { value: "date", label: "Date" },
  { value: "email", label: "Email" },
  { value: "phone", label: "Phone" },
  { value: "url", label: "URL" }
];

// Common dropdown options for specific fields
export const DROPDOWN_OPTIONS = {
  "VLAN_Support": ["Yes", "No"],
  "PoE_Support": ["Yes", "No"],
  "WiFi_Capability": ["Yes", "No"],
  "Surge_Protection": ["Yes", "No"], 
  "Battery_Integration": ["Yes", "No"],
  "Rack_Mount": ["Yes", "No"],
  // GP-specific dropdown options
  "ONTs_Installed": ["Yes", "No"],
  "litup_Yes_No": ["Yes", "No"],
  "Phase_1_or_2": ["Phase 1", "Phase 2"],
  "OTDR_status": ["Active", "Inactive", "Pending"],
  "Ring_topology": ["Star", "Ring", "Bus", "Mesh"],
  "Fiber_core_12_24_48_96": ["12", "24", "48", "96"],
  // OFC-specific dropdown options
  "FPOI_Closure_Available": ["Yes", "No"]
}; 