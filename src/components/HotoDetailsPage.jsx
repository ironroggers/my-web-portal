...
import { fetchAllHotoList, fetchLocationDetails, uploadHotoMedia, createHoto, deleteHoto } from '../services/hotoPageService';
...
const HotoDetailsPage = () => {
  ...
  const applyFilters = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(hoto =>
        (hoto.hotoType || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
        (hoto.blockName || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
        (hoto.gpName || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
        (hoto.ofcName || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
        (hoto.remarks || '').toLowerCase().includes(searchTerm.toLowerCase()) ||
        (hoto.fields || []).some(field => 
    ...
    if (hasMediaFilter) {
      filtered = filtered.filter(hoto => {
        const hasMedia = (hoto.fields || []).some(field => field.mediaFiles && field.mediaFiles.length > 0);
    ...
    if (dateFilter) {
      ...
      if (filterDate) {
        filtered = filtered.filter(hoto => 
          new Date(hoto.createdAt) >= filterDate
        );
      }
    }
  ...
  const getHotoStats = () => {
    return {
      ...
      totalMedia: hotos.reduce((acc, h) => 
        acc + (h.fields?.reduce((facc, f) => facc + (f.mediaFiles?.length || 0), 0) || 0), 0
      )
    };
  };
    ...
    return (
      <Container maxWidth="xl" sx={{ py: 4 }}>
        <Box display="flex" justifyContent="center" alignItems="center" minHeight="400px">
          ...
          <Typography variant="h6" sx={{ ml: 2 }}>Loading HOTO details...</Typography>
        </Box>
      </Container>
    );
  }
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 3 }}>
      {/* Header */}
      <Paper elevation={0} sx={{ p: 3, mb: 3, background: 'linear-gradient(135deg, #4CAF50 0%, #2E7D32 100%)', color: 'white', borderRadius: 2 }}>
        ...
        <Box display="flex" alignItems="center" justifyContent="space-between" flexWrap="wrap" gap={2}>
          ...
          <Box display="flex" gap={2}>
            ...
            <Paper sx={{ 
              ...
              <Typography variant="caption" color="white" display="block">Total HOTOs</Typography>
              <Typography variant="h6" color="white" fontWeight="bold">{stats.totalHotos}</Typography>
            </Paper>
            <Paper sx={{ 
              p: 1.5, 
              bgcolor: 'rgba(255,255,255,0.1)', 
              ...
              <Grid container spacing={2}>
                <Grid xs={6} sm={3}>
                  <Paper elevation={1} sx={{ 
                    ...
                    <Typography variant="h5" color="primary.main" fontWeight="bold">{stats.blockHotos}</Typography>
                ...
                <Grid xs={6} sm={3}>
                  <Paper elevation={1} sx={{ 
                    ...
                    <Typography variant="h5" color="secondary.main" fontWeight="bold">{stats.gpHotos}</Typography>
                ...
                <Grid xs={6} sm={3}>
                  <Paper elevation={1} sx={{ 
                    ...
                    <Typography variant="h5" color="success.main" fontWeight="bold">{stats.ofcHotos}</Typography>
            ...
            <>
              <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
                <Typography variant="h6" color="primary">
                  🏢 Showing {filteredHotos.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).length} of {filteredHotos.length} HOTO entries
                </Typography>
                  ...
                  .map((hoto) => (
                    ...
                    >
                      <AccordionSummary
                        ...
                        sx={{ 
                          bgcolor: expandedHotos[hoto._id] ? 'primary.50' : 'white',
                          '&:hover': { bgcolor: expandedHotos[hoto._id] ? 'primary.100' : 'grey.50' },
                          borderRadius: expandedHotos[hoto._id] ? '8px 8px 0 0' : 2,
                          minHeight: '80px',
                      ...
                      >
                        <Box display="flex" justifyContent="space-between" alignItems="center" width="100%" py={1}>
                          <Box display="flex" alignItems="center" gap={3}>
                            <Avatar sx={{ 
                              bgcolor: hoto.hotoType === 'block' ? 'primary.main' : 
                                       hoto.hotoType === 'gp' ? 'secondary.main' : 'success.main',
                              width: 48,
                              height: 48
                            }}>
                              <BusinessIcon />
                            ...
                            <Box>
                              <Typography variant="h6" fontWeight="bold" color="text.primary" sx={{ mb: 0.5 }}>
                                {hoto.hotoType?.toUpperCase()} - {hoto.blockName || hoto.gpName || hoto.ofcName || 'Unknown'}
                              </Typography>
                              <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.875rem' }}>
                                {hoto.remarks || (hoto.contactPerson?.name ? `Contact: ${hoto.contactPerson.name}` : 'No additional details')}
                              </Typography>
                            </Box>
                          </Box>
                          ...
                          <Box display="flex" alignItems="center" gap={1} onClick={(e) => e.stopPropagation()}>
                            <Chip
                              ...
                              color={hoto.hotoType === 'block' ? 'primary' : 
                                     hoto.hotoType === 'gp' ? 'secondary' : 'success'}
                              size="small"
                              sx={{ 
                                fontWeight: 600,
                                minWidth: '70px',
                            ...
                            />
                            <Chip
                              label={`${(hoto.fields?.reduce((acc, f) => acc + (f.mediaFiles?.length || 0), 0) || 0)} Media`}
                              size="small"
                              variant="outlined"
                              color="info"
                              sx={{ 
                        ...
                        <Grid container spacing={3}>
                              ...
                              <List dense>
                                  ...
                                  <>
                                    <ListItem sx={{ px: 0 }}>
                                      ...
                                      <ListItemText 
                                        ...
                                        secondary={hoto.contactPerson.name || 'N/A'} 
                                      />
                                    </ListItem>
                                ...
                                {hoto.latitude && hoto.longitude && (
                                  <ListItem sx={{ px: 0 }}>
                                    ...
                                    <ListItemText 
                                      primary="Coordinates" 
                                      secondary={`${formatCoordinate(hoto.latitude)}, ${formatCoordinate(hoto.longitude)}`} 
                                    />
                                  </ListItem>
                                )}
                              </List>
                          ...
                          <Grid xs={12} md={8}>
                            ...
                            {hoto.fields && hoto.fields.length > 0 ? (
                                ...
                                {hoto.fields.map((field, fieldIndex) => (
                                  <Paper key={fieldIndex} elevation={1} sx={{ p: 2, border: '1px solid #e0e0e0' }}>
                                    <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={2}>
                                      ...
                                      <Stack direction="row" spacing={1} sx={{ minWidth: '280px', justifyContent: 'flex-end' }}>
                                        <Chip 
                                        ...
                                        {field.confirmation !== undefined && (
                                          <Chip 
                                            icon={field.confirmation ? <CheckCircleIcon /> : <CancelIcon />}
                                            label={field.confirmation ? 'Confirmed' : 'Not Confirmed'} 
                                            size="small" 
                                            color={field.confirmation ? 'success' : 'error'}
                                            variant="outlined"
                                            sx={{ 
                                              minWidth: '120px',
                                              height: '28px'
                                        ...
                                        {field.status !== undefined && (
                                          <Chip 
                                            label={field.status === 1 ? 'Active' : 'Inactive'} 
                                            size="small" 
                                            color={field.status === 1 ? 'success' : 'default'}
                                            variant="outlined"
                                            sx={{ 
                                              minWidth: '80px',
                                              height: '28px'
                                    ...
                                    {field.remarks && (
                                      <Typography variant="body2" sx={{ mb: 2, bgcolor: 'info.50', p: 1, borderRadius: 1 }}>
                                        <strong>Remarks:</strong> {field.remarks}
                                      </Typography>
                                    )}
                                          ...
                                          {field.mediaFiles.map((media, mediaIndex) => (
                                            <Grid xs={6} sm={4} md={3} key={mediaIndex}>
                                              ...
                                              >
                                                ...
                                                <Box sx={{ mt: 1 }}>
                                                  <Typography variant="caption" display="block" fontWeight="bold" sx={{ 
                                                    overflow: 'hidden',
                                                  ...
                                                  {media.description && (
                                                    <Typography variant="caption" color="text.secondary" display="block" sx={{
                                                      ...
                                                      {media.description.length > 15 ? 
                                                        `${media.description.substring(0, 15)}...` : 
                                                        media.description
                                                      }
                                                    </Typography>
                                                  )}
                                                  {media.place && (
                                                    <Typography variant="caption" color="info.main" display="block" sx={{
                                                      overflow: 'hidden',
                                                      textOverflow: 'ellipsis',
                                                      whiteSpace: 'nowrap',
                                                      fontSize: '0.65rem'
                                                    }}>
                                                      📍 {media.place.length > 12 ? 
                                                        `${media.place.substring(0, 12)}...` : 
                                                        media.place
                                                      }
                                                    </Typography>
                                                  )}
                              ...
                              <Alert severity="info">No fields found for this HOTO entry.</Alert>
                            )}
                          </Grid>
                        </Grid>
                      </AccordionDetails>
              ...
              <Grid xs={12} md={6}>
                ...
                <List dense>
                  <ListItem sx={{ px: 0 }}>
                    ...
                    <ListItemText 
                      primary="Description" 
                      secondary={selectedMedia.description || 'No description available'} 
                    />
                  </ListItem>
                  <ListItem sx={{ px: 0 }}>
                  ...
                  <ListItem sx={{ px: 0 }}>
                    ...
                    <ListItemText 
                      ...
                      secondary={selectedMedia.deviceName || selectedMedia.source || 'Unknown'} 
                    />
                  </ListItem>
                </List>
                ...
                <List dense>
                  {(selectedMedia.latitude || selectedMedia.longitude) ? (
                    <>
                      <ListItem sx={{ px: 0 }}>
                        <ListItemIcon><GPSIcon fontSize="small" color="success" /></ListItemIcon>
                        <ListItemText 
                          primary="GPS Coordinates" 
                          secondary={`${formatCoordinate(selectedMedia.latitude)}, ${formatCoordinate(selectedMedia.longitude)}`}
                        />
                      </ListItem>
                      ...
                      <ListItem sx={{ px: 0 }}>
                        <Button
                          ...
                          disabled={!selectedMedia.latitude || !selectedMedia.longitude}
                          fullWidth
                        >
                          View on Google Maps
                        </Button>
        ...
        <DialogContent>
          ...
          {hotoToDelete && (
            <Box sx={{ mt: 2, p: 2, bgcolor: 'grey.50', borderRadius: 1 }}>
              <Typography variant="subtitle2" fontWeight="bold">
                {hotoToDelete.hotoType?.toUpperCase()} - {hotoToDelete.blockName || hotoToDelete.gpName || hotoToDelete.ofcName}
              </Typography>
              <Typography variant="body2" color="text.secondary">
                Created: {formatDate(hotoToDelete.createdAt)}
              </Typography>
              <Typography variant="body2" color="text.secondary">
                Fields: {hotoToDelete.fields?.length || 0} | 
                Media: {hotoToDelete.fields?.reduce((acc, f) => acc + (f.mediaFiles?.length || 0), 0) || 0}
              </Typography>
            </Box>
          )}
        </DialogContent>
        <DialogActions sx={{ p: 3 }}>
          <Button onClick={handleCloseDeleteConfirm} variant="outlined" disabled={deletingHoto}>
            Cancel
          </Button>
          <Button 
            ...
            startIcon={deletingHoto ? <CircularProgress size={16} /> : <DeleteIcon />}
          >
